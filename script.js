document.addEventListener('DOMContentLoaded', () => {
  ...
  const PYTHON_SERVICE_API = 'https://afdmi-123.onrender.com:5001'; // Python langextract_service.py
  const NODE_API = process.env.NEXT_PUBLIC_API_URL || 'https://afdmi-123.onrender.com:5001'; // Node.js backend for other operations
  ...
  function handleFileSelect(files) {
    ...
    fileList.innerHTML = ingestedFiles.map(f => `<li>${f.name} (${(f.size / 1024).toFixed(1)} KB)</li>`).join('');
  ...
  async function processUploadedFiles(modelType, params) {
    const file = ingestedFiles[0]; // Process first file for now
    const formData = new FormData();
    formData.append('file', file);
    formData.append('prompt', getPromptForModel(modelType, params));
    
    processingLogs.textContent = 'Processing file with AI model...';
