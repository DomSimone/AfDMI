...
const JWT_SECRET = '521dc52355d38c229643b0d2bbe3fd47';
...
// UPDATED: Pointing to your Render URL instead of localhost
...
const server = http.createServer((req, res) => {
    ...
    // 1. Document Ingestion
        ...
        bb.on('file', (fieldname, file, filename, encoding, mimetype) => {
            ...
            file.on('end', () => {
                let safeFilename = typeof filename === 'string' ? filename : (filename?.filename || "unknown");
        ...
        bb.on('finish', () => {
            ...
            res.end(JSON.stringify({ success: true, files: filePromises }));
        });
        req.pipe(bb);
    }

    // 2. Document Extraction (Connected to Render Python Service)
        ...
        req.on('end', async () => {
            try {
                ...
                if (!buffer) {
                    res.writeHead(400, { 'Content-Type': 'application/json' });
                    return res.end(JSON.stringify({ error: 'No document content provided.' }));
                }

                // Constructing the multipart/form-data for the Python Service
                ...
                try {
                    // Calling your Render Python Host
                    ...
                    if (!pythonServiceRes.ok) throw new Error(pythonResult.error || 'Python service error');

                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ data: pythonResult, outputFormat }));

                } catch (fetchError) {
                    console.error("Connection to Render Python failed:", fetchError.message);
                    // Fallback to mock logic
                    const mockRes = mockExtractFromDocuments({
                        files: [{ fileName: safeFilename, buffer, mimetype: 'application/pdf' }],
                        prompt: prompt,
                        outputFormat: outputFormat || 'json'
                    });
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ data: mockRes.data, warning: "Using fallback mock data." }));
                }
            } catch (e) {
